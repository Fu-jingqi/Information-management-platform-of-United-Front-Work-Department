...
public class PersonInforDaoImpl extends BaseDao implements PersonInforDao {
    ...
    public int UpdatePersonById(PersonInfor person) {
        String sql = "update workpersoninfor set pname=?,psex=?,pbirth=?,pnational=?,pnativeplace=?,ppoliticalstatus=?,pworkplace=?,pxueli=?,pxuewei=?,pzhicheng=?,pcurrentposition=?,pTenureOfTheSameRank=?,pLengthOfTenure=?,psocialwork=?,pjoinworktime=?,pjoinpartytime=?,phonor=?,ptraining=?,pcanzhengyizheng=?,pcontactnum=?,pinfor=? where pid=?" ;
        return this.modifyData(sql,new Object[] {person.getPname(),person.getPsex(),person.getPbirth(),person.getPnational(),person.getPnativeplace(),person.getPpoliticalstatus(),person.getPworkplace(),person.getPxueli(),person.getPxuewei(),person.getPzhicheng(),person.getPcurrentposition(),person.getpTenureOfTheSameRank(),person.getpLengthOfTenure(),person.getPsocialwork(),person.getPjoinworktime(),person.getPjoinpartytime(),person.getPhonor(),person.getPtraining(),person.getPcanzhengyizheng(),person.getPcontactnum(),person.getPinfor(),person.getPid()}) ;
    }

    @Override
    public int AddPerson(PersonInfor person) {
        String sql = "insert into workpersoninfor (pname, psex, pbirth,pnational,pnativeplace,ppoliticalstatus,pworkplace,pxueli,pxuewei,pzhicheng,pcurrentposition,pTenureOfTheSameRank,pLengthOfTenure,psocialwork,pjoinworktime,pjoinpartytime,phonor,ptraining,pcanzhengyizheng,pcontactnum,pinfor) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)" ;
        return this.modifyData(sql,new Object[] {person.getPname(),person.getPsex(),person.getPbirth(),person.getPnational(),person.getPnativeplace(),person.getPpoliticalstatus(),person.getPworkplace(),person.getPxueli(),person.getPxuewei(),person.getPzhicheng(),person.getPcurrentposition(),person.getpTenureOfTheSameRank(),person.getpLengthOfTenure(),person.getPsocialwork(),person.getPjoinworktime(),person.getPjoinpartytime(),person.getPhonor(),person.getPtraining(),person.getPcanzhengyizheng(),person.getPcontactnum(),person.getPinfor()}) ;
    }

    public List<PersonInfor> getPersonByPage(int pageNo ,int pageSize){
        // (pageNo - 1) * pageSize
        String sql = "select * from workpersoninfor limit ?,?";
    ...
    public int GetPersonCount() {
        String sql = "select count(1) from workpersoninfor" ;
        ResultSet rs = this.getDataByAny(sql,new Object[] {}) ;
        int count = 0 ;
        try {
            while(rs.next()){
